...
DEFAULT_DATAKIT_SHEET = "myWorkSheet"
...
<style>
    /* Header */
    .dtm-title { color: #1F3864; font-size: 1.9rem; font-weight: 700; margin-bottom: 0.1rem; }
    .dtm-sub   { color: #666;   font-size: 0.88rem; margin-top: 0; }
    ...
    .sidebar-section { font-weight: 600; color: #1F3864; margin-top: 0.5rem; }

    /* Make upload widget a bit tighter */
    section[data-testid="stFileUploader"] { padding-top: 0; }
</style>
...
with st.sidebar:
    ...
    uploaded_dk = None
    datakit_sheet = DEFAULT_DATAKIT_SHEET
    ...
    formcomponents = st.text_input(
        "Component name",
        value="Baseline Sub-Area Assessment",
    ...
    with st.expander("🗂 Column mappings"):
        ...
        df1_key_col = st.text_input("Survey — key field",   value="name")
        df1_text_col = st.text_input("Survey — label field", value="label")
        df1_type_col = st.text_input("Survey — type field",  value="type")
        st.markdown("---")
        df2_key_col = st.text_input(
            "Datakit — key field",       value="FieldName")
        df2_text_col = st.text_input(
            "Datakit — question text",    value="QuestionText(en)")
        df2_id_col = st.text_input(
            "Datakit — unique ID",        value="FieldUniqueId")
        df2_type_col = st.text_input(
            "Datakit — answer type",      value="QuestionAnswerType")
        df2_comp_col = st.text_input(
            "Datakit — component column", value="QuestionComponent")
